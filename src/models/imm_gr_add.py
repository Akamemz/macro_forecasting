...
Architecture (TIME-IMM Appendix I.2, eq. 12-16):
  ...
  2. RecAvgTTF        →  e     [B, TEXT_EMB_DIM]  (text context)
  3. z = [y_ts ; e]
  4. H = GRU({z})
  5. ΔY = W_Δ · H
  6. G  = σ(W_g · z)
...
from src.config import TEXT_EMB_DIM, SIGMA, N_INDICATORS, CONTEXT_WINDOW, TARGET_COLS, HIDDEN_DIM
...
class IMMGRAddMMF(nn.Module):
    ...
    def __init__(
        ...
        # TTF: zero-parameter Gaussian text fusion
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        # ── step 2: text context via RecAvg TTF ──────────────────────────────
        e = self.ttf(

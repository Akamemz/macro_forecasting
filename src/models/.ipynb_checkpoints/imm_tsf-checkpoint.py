...
def _patch_latent_ode():
    """Fix tensor boolean ambiguity bug in IMM-TSF without modifying source files."""
    try:
        import importlib
        import lib.latent_ode_components.encoder_decoder as _ed
        import inspect
        src = inspect.getsource(_ed)
        patched = src.replace(
            "if (prev_t - t_i) < minimum_step:",
            "if (prev_t - t_i).item() < minimum_step:",
        )
        if patched != src:
            exec(compile(patched, _ed.__file__, "exec"), vars(_ed))
    except Exception:
        pass
